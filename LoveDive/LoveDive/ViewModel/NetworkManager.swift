...
class NetworkManager {
  ...
  func getCurrentWeatherData(lat: Double, lng: Double) {
      ...
      !weatherCache.weather.isEmpty
    {
      ...
      Task {
        do {
          try await getFromFirebase(lat: lat, lng: lng)
        }
        catch {
          self.getFromAPI(lat: lat, lng: lng)
        }
      }
    }
  }
  ...
  func getFromFirebase(lat: Double, lng: Double) async throws {
    let key = "currentWeather\(lat),\(lng)"
    let data = try await weatherDocument(lat: lat, lng: lng).getDocument(as: WeatherData.self)
    self.currentDelegate?.manager(didGet: data.hours, forKey: key)
    self.saveToUserDefault(value: data, key: key)
  }
  
  func getFromAPI(lat: Double, lng: Double) {
